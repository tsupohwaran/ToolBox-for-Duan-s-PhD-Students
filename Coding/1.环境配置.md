...
4. 回到GitHub页面，可以看到刚刚推送的代码已经出现在远程仓库中。页面中提示我们“Compare & pull request”，表示可以审查，并决定是否要将分支代码合并到主分支。例如，小红书要开发一个新功能，程序员将在新分支上进行开发，完成后需要提交合并请求（Pull Request）给产品经理审查。产品经理可以在GitHub上查看代码更改，并决定是否合并到主分支并向用户推送。

>**注意**：如果未在VS Code中登录GitHub账号，则会提示输入GitHub账号和密码。
